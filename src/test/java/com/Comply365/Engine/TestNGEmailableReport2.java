...
public class TestNGEmailableReport2 implements IReporter {
	...
	private static final Logger LOG = Logger.getLogger(TestNGEmailableReport2.class);
	...
	protected void writeReportTitle(String title) {
		writer.println("<center><h1>" + title + " - " + CommonMethod.getCurrentDateTime() + "</h1></center>");
	}
	...
	protected void writeStylesheet() {
		...
		writer.print("table {border: 1px solid black;border-style: solid;margin-bottom:10px;border-collapse:collapse;empty-cells:show}");
		writer.print("#summary {margin-top:30px}");
		writer.print("h1 {font-size:30px}");
		writer.print("body {width:100%;}");
		writer.print("th,td {border: 1px solid black;border-style: solid;padding: 8px}");
		...
		writer.print(".easy-overview {border: 1px solid black;border-style: solid;margin-left: auto; margin-right: auto;} ");
		writer.print(".easy-test-overview tr:first-child {border: 1px solid black;border-style: solid;background-color:#D3D3D3}");
	 ...
	 * writer.print("<thead>"); writer.print("<tr>");
	 * writer.print("<th>Class</th>"); writer.print("<th>Method</th>");
	 * writer.print("<th>Short Exception</th>");
	 * writer.print("<th>Screenshot</th>"); writer.print("<th>Start Time</th>");
	 * writer.print("<th>End Time</th>"); writer.print("</tr>");
	 * writer.print("</thead>");
	 * 
	 * int testIndex = 0; int scenarioIndex = 0; for (SuiteResult suiteResult :
	 * suiteResults) { writer.print("<tbody><tr><th colspan='6'>");
	 * writer.print(Utils.escapeHtml(suiteResult.getSuiteName()));
	 ...
	 * 
	 * String testName = Utils.escapeHtml(testResult.getTestName()); int startIndex
	 * = scenarioIndex;
	 * 
	 ...
	 * scenarioIndex += writeScenarioSummary(testName + " &#8212; failed",
	 * testResult.getFailedTestResults(), "failed", scenarioIndex); scenarioIndex +=
	 * writeScenarioSummary(testName + " &#8212; skipped (configuration methods)",
	 * testResult.getSkippedConfigurationResults(), "skipped", scenarioIndex);
	 * scenarioIndex += writeScenarioSummary(testName + " &#8212; skipped",
	 * testResult.getSkippedTestResults(), "skipped", scenarioIndex); scenarioIndex
	 * += writeScenarioSummary(testName + " &#8212; passed",
	 * testResult.getPassedTestResults(), "passed", scenarioIndex);
	 * 
	 ...
	 * testIndex++; } }
	 * 
	 * writer.println("</table>"); writer.println("</div>"); }
	 * 
	 *//**
		 ...
		 */
	/*
	 * private int writeScenarioSummary(String description, List<ClassResult>
	 * classResults, String cssClassPrefix, int startingScenarioIndex) { int
	 * scenarioCount = 0; if (!classResults.isEmpty()) {
	 * writer.print("<tr><th colspan="6">"); writer.print(description);
	 * writer.print("</th></tr>");
	 * 
	 * int scenarioIndex = startingScenarioIndex; int classIndex = 0; for
	 * (ClassResult classResult : classResults) { String cssClass = cssClassPrefix +
	 * ((classIndex % 2) == 0 ? "even" : "odd");
	 * 
	 * buffer.setLength(0);
	 * 
	 * int scenariosPerClass = 0; int methodIndex = 0; for (MethodResult
	 * methodResult : classResult.getMethodResults()) { List<ITestResult> results =
	 * methodResult.getResults(); int resultsCount = results.size(); assert
	 * resultsCount > 0;
	 * 
	 * ITestResult firstResult = results.iterator().next(); String methodName =
	 * Utils.escapeHtml(firstResult .getMethod().getMethodName()); long start =
	 * firstResult.getStartMillis(); long end = firstResult.getEndMillis();
	 * 
	 * String shortException=""; String failureScreenShot="";
	 * 
	 * Throwable exception=firstResult.getThrowable(); boolean hasThrowable =
	 * exception != null; if(hasThrowable){ String str =
	 * Utils.shortStackTrace(exception, true); Scanner scanner = new Scanner(str);
	 * shortException = scanner.nextLine(); scanner.close(); List<String> msgs =
	 * Reporter.getOutput(firstResult); boolean hasReporterOutput = msgs.size() > 0;
	 * if(hasReporterOutput){ for (String info : msgs) {
	 * failureScreenShot+=info+"<br/>"; } } }
	 * 
	 * 
	 * DateFormat formatter = new SimpleDateFormat("hh:mm:ss a"); Calendar startTime
	 * = Calendar.getInstance(); startTime.setTimeInMillis(start);
	 * 
	 * Calendar endTime = Calendar.getInstance(); endTime.setTimeInMillis(end);
	 * 
	 * 
	 * // The first method per class shares a row with the class // header if
	 * (methodIndex > 0) { buffer.append("<tr class="").append(cssClass)
	 * .append("">");
	 * 
	 * }
	 * 
	 ...
	 * .append("">").append(methodName) .append("</a></td>").append("<td rowspan="")
	 * .append("">").append(shortException) .append("</td>").append("<td rowspan="")
	 ...
	 * ); scenarioIndex++;
	 * 
	 * // Write the remaining scenarios for the method for (int i = 1; i <
	 * resultsCount; i++) { buffer.append("<tr class="").append(cssClass)
	 * .append("">").append("<td><a href="#m") .append(scenarioIndex).append("">")
	 * .append(methodName).append("</a></td></tr>"); scenarioIndex++; }
	 * 
	 * scenariosPerClass += resultsCount; methodIndex++; }
	 * 
	 * // Write the test results for the class writer.print("<tr class="");
	 * writer.print(cssClass); writer.print("">"); writer.print("<td rowspan="");
	 * writer.print(scenariosPerClass); writer.print("">");
	 * writer.print(Utils.escapeHtml(classResult.getClassName()));
	 * writer.print("</td>"); writer.print(buffer);
	 * 
	 * classIndex++; } scenarioCount = scenarioIndex - startingScenarioIndex; }
	 * return scenarioCount; }
	 * 
	 *//**
		 * Writes the details for all test scenarios.
		 *//*
			 * protected void writeScenarioDetails() { int scenarioIndex = 0; for
			 * (SuiteResult suiteResult : suiteResults) { for (TestResult testResult :
			 * suiteResult.getTestResults()) { writer.print("<h2>");
			 * writer.print(Utils.escapeHtml(testResult.getTestName()));
			 * writer.print("</h2>");
			 * 
			 * scenarioIndex += writeScenarioDetails(
			 * testResult.getFailedConfigurationResults(), scenarioIndex); scenarioIndex +=
			 * writeScenarioDetails( testResult.getFailedTestResults(), scenarioIndex);
			 * scenarioIndex += writeScenarioDetails(
			 * testResult.getSkippedConfigurationResults(), scenarioIndex); scenarioIndex +=
			 * writeScenarioDetails( testResult.getSkippedTestResults(), scenarioIndex);
	 ...
	 */
	/*
	 * private int writeScenarioDetails(List<ClassResult> classResults, int
	 * startingScenarioIndex) { int scenarioIndex = startingScenarioIndex; for
	 * (ClassResult classResult : classResults) { String className =
	 * classResult.getClassName(); for (MethodResult methodResult :
	 * classResult.getMethodResults()) { List<ITestResult> results =
	 * methodResult.getResults(); assert !results.isEmpty();
	 * 
	 * String label = Utils .escapeHtml(className + "#" +
	 * results.iterator().next().getMethod() .getMethodName()); for (ITestResult
	 * result : results) { writeScenario(scenarioIndex, label, result);
	 * scenarioIndex++; } } }
	 * 
	 ...
	 */
	/*
	 * private void writeScenario(int scenarioIndex, String label, ITestResult
	 * result) { writer.print("<h3 id="m"); writer.print(scenarioIndex);
	 * writer.print("">"); writer.print(label); writer.print("</h3>");
	 * 
	 * writer.print("<table class="table-bordered result">");
	 * 
	 * boolean hasRows = false;
	 * 
	 * // Write test parameters (if any) Object[] parameters =
	 * result.getParameters(); int parameterCount = (parameters == null ? 0 :
	 * parameters.length); if (parameterCount > 0) {
	 * writer.print("<tr class="param">"); for (int i = 1; i <= parameterCount; i++)
	 * { writer.print("<th>Parameter #"); writer.print(i); writer.print("</th>"); }
	 ...
	 * writer.print("</td>"); } writer.print("</tr>"); hasRows = true; }
	 * 
	 * // Write reporter messages (if any) List<String> reporterMessages =
	 * Reporter.getOutput(result); if (!reporterMessages.isEmpty()) {
	 ...
	 * writer.printf(" colspan="%d"", parameterCount); } writer.print(">");
	 * writeReporterMessages(reporterMessages); writer.print("</td></tr>"); hasRows
	 * = true; }
	 * 
	 * // Write exception (if any) Throwable throwable = result.getThrowable(); if
	 * (throwable != null) { writer.print("<tr><th"); if (parameterCount > 1) {
	 * writer.printf(" colspan="%d"", parameterCount); } writer.print(">");
	 * writer.print((result.getStatus() == ITestResult.SUCCESS ?
	 * "Expected Exception" : "Exception")); writer.print("</th></tr>");
	 * 
	 * writer.print("<tr><td"); if (parameterCount > 1) {
	 * writer.printf(" colspan="%d"", parameterCount); } writer.print(">");
	 * writeStackTrace(throwable); writer.print("</td></tr>"); hasRows = true; }
	 * 
	 * if (!hasRows) { writer.print("<tr><th"); if (parameterCount > 1) {
	 * writer.printf(" colspan="%d"", parameterCount); }
